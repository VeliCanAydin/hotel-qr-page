...
- [ ] **Public içerik cache'i** — `force-dynamic` public sayfaları `use cache` + `cacheTag`'e
  geçir; admin action'lardaki `revalidatePath` çağrılarını tag invalidation'a çevir.
  1000 misafir senaryosundaki okuma yükünün büyük kısmını DB'den koparır.
  *(Menü müsaitliği maddesinden önce yapılmalı — invalidation deseni burada kurulacak.)*
