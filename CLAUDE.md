...
- **Etkinlik push'ları:** yeni etkinlik oluşturulunca `createEvent` tüm aktif (in-house) misafirlere kendi dillerinde duyuru yollar (`sendPushToActiveGuests`). Başlamadan ≤60 dk önce hatırlatma: Vercel Cron (`vercel.json`, 15 dk'da bir) → `GET /api/cron/event-reminders` (`CRON_SECRET` Bearer auth, fail-closed). Mükerrer koruması `events.reminder_sent_at`; `updateEvent` tarih/saat değişince sıfırlar. Hatırlatma metinleri `content_translations`'taki event çevirilerini kullanır.
