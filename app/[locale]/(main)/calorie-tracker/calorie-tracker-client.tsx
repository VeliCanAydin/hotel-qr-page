...
import { Flame, Upload, Camera, Trash2, Check, Sparkles, AlertCircle, RefreshCw, BarChart2, Lock, Info, ArrowLeft, History, Edit3, X } from 'lucide-react';
...
export default function CalorieTrackerClient({ isAuthorized }: { isAuthorized: boolean }) {
    ...
    return (
        <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 p-4 pb-24 text-foreground transition-all duration-300">
                ...
                <div className="max-w-xl mx-auto w-full transition-all duration-300">
                    <Card className="border-border rounded-3xl shadow-sm overflow-hidden bg-card">
                        ...
                        <CardContent className="flex flex-col gap-5">
                            ...
                            >
                                {!isAuthorized ? (
                                    <div className="flex flex-col items-center justify-center gap-4 text-center p-4 w-full">
                                        ...
                                        <span className="text-sm text-muted-foreground text-center max-w-sm">
                                            {t.rich("lockedNotice", {
                                                link: (chunks) => (
                                                    <Link
                                                        href="/login?redirect=/calorie-tracker"
                                    ...
                                    <div className="flex flex-col items-center justify-center gap-3 text-center p-4 w-full">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md mt-2">
                                            <label
                                                className={`flex-1 relative bg-[#45a7d7] hover:bg-[#45a7d7]/90 text-white rounded-xl font-bold flex items-center justify-center gap-1.5 py-2.5 cursor-pointer text-center select-none text-sm ${isAnalyzing ? 'opacity-50 pointer-events-none' : ''}`}
                                            >
            ...
            {isAuthorized && (todayLogs.length > 0 || analysisResult) && (
